...
<body>
    <div class="container">
         <form action="${pageContext.request.contextPath}/Role/doUpdate" method="post" id="roleEditForm" class="form-horizontal">
            <!-- 角色编号 -->
            <input type="hidden" value="${role.roleId}" id="roleId" name="roleId">
            <input type="hidden" value="${role.name}" id="oldName" name="oldName">
            <fieldset>
               <legend >
                    <label >
                ...
                <div class="form-group" id="noteDiv">
                    ...
                    <div class="col-sm-5">
                        <input type="text" name="note" id="note" class="form-control"
                               placeholder="请输入角色备注" value="${role.note}">
                    </div>
                   ...
                   <div class="col-sm-5">
                       ...
                       <c:forEach items="${allPermissions}" var="perm">
                           ...
                           <label class="checkbox-inline">
	                           <c:forEach items="${role.permissions}" var="myPerm">
	                               <!-- 选中我自己之前的权限 -->
	                               <c:if test="${perm.permissionId==myPerm.permissionId}">
	                                  <input type="checkbox"  name="permissionId" value="${perm.permissionId}" checked="checked">
	                                   ${perm.note} 
	                                  <!-- 修改标记位,再定夺循环几次无果 -->
	                           ...
	                           <!-- 内层循环没有一致的权限,那么复选框直接打印即可 -->
								<c:if test="${checked==0}">
								   <input type="checkbox"  name="permissionId" value="${perm.permissionId}">
								    ${perm.note}
								</c:if>
                           </label>
