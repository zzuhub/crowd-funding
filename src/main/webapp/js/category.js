$(function(){
    ...
    $("#categoryInsertForm").validate({    //定义验证规则
		    	      ...
		    	      "name" :{
		    	    	       ...
		    	    	       remote : {
			   	    	    	   ...
			   	    	    	   dataFilter :function(jsonStr,type){       //处理完成执行的函数
			   	    	    		      ...
			   	    	    		      if(json.status==0){  //AJAX验证类别名不存在,可用！
			   	    	    		    	  flag=true   ;
			   	    	    		      }
			   	    	    		      return flag ;
			   	    	    	   }
	   ...
	   submitHandler:function(form){
	                 //我们在这里使用AJAX提交表单
		             var url=$("#categoryInsertForm").attr("action")  ;   //获取Action
...
//===================================ucategory_insert.jsp end==============================================
  ...
  //===================================category_edit.jsp start==============================================      
    $("#categoryEditForm").validate({    //定义验证规则
	      debug : false   ,   //采用调试模式,表单不会自动提交 
	      errorPlacement : function(error,element){   //error表示自动生成的错误信息文字,element表示触发的元素(JQuery对象)
	    	       //调试专用语句  console.log("#"+element.attr("id")+"Msg");
	    	           //一行三目运算符搞定
	    	      ...
	    	      "name" : {
	    	    	     ...
	    	    	     remote : {
	    	    	    	   ...
	    	    	    	   dataFilter :function(jsonStr,type){       //处理完成执行的函数
	    	    	    		      var flag=false ;
	    	    	    		      var json=$.parseJSON(jsonStr);
	    	    	    		      var oldName=$("#oldName").val();  //源类别名
	    	    	    		      var newName=$("#name").val(); //新类别名
	   ...
	   submitHandler:function(form){
	                 //我们在这里使用AJAX提交表单
		             var url=$("#categoryEditForm").attr("action")  ;   //获取Action
		             var id=$("#id").val() ;   //类别ID
		             var name=$("#name").val() ;   //类别名称
		             $.ajax({
